...
discussed further in \autoref{sec:methods}.
...
\begin{figure}[H]
    ...
    \caption{YOLO26 Architecture Diagram. Reproduced from \cite{hidayatullah2026yolo26}, available under \href{https://creativecommons.org/licenses/by/4.0/}{CC BY 4.0}.}
...
The fundamental building block for evaluating detection accuracy is Intersection over Union (IoU). This metric measures the overlap between the predicted bounding box (\(B_p\)) and the ground truth box (\(B_gt\)) provided during the manual annotation. It is calculated as:
...
The literature commonly reports two versions of mAP to provide a complete picture of model performance. The mAP$_{50}$ represents the mAP calculated at a fixed IoU treshold of 0.5. This means that a detection is labeled as a True Positive if the overlap between the predicted box and the GT box is minimum 50\%. Instead of using a single treshold, mAP$_{50}$-95 uses the average of mAP values calculated at ten different IoU tresholds from 0.5 to 0.95, in 0.05 increments. This metric rewards models for superior localization accuracy.
